...
$section = $sectionId > 0
    ? CIBlockSection::GetList([], ['ID' => $sectionId, 'IBLOCK_ID' => 18, 'CHECK_PERMISSIONS' => 'Y'], false, ['ID'])->Fetch()
    : false;

if (!$section) {
    ShowError('В компоненте «Прикреплённые файлы» не выбран раздел или выбран раздел другого инфоблока.');
    return;
}
